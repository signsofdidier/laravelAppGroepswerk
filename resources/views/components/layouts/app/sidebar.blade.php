...
<flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
    ...
    <flux:navlist variant="outline">
        ...
        <flux:navlist.item icon="credit-card" :href="route('pricing')" :current="request()->routeIs('pricing')"
                           wire:navigate> {{ __('Pricing') }}
        </flux:navlist.item>

        <flux:navlist.item icon="document-text" :href="route('orders.index')"
    ...
    <flux:dropdown position="bottom" align="start">
        ...
        <flux:menu class="w-[220px]">
            ...
            @if (auth()->user()->latestOrder)
                <div class="mt-4 p-4 bg-white rounded shadow"><p>
                        <strong>Plan:</strong> {{ auth()->user()->latestOrder->plan->name }}</p></div>
            @else
